...
def train_main(model_args=None, data_args=None, training_args=None):
    parser = HfArgumentParser((ModelArguments, DataArguments, TrainingArguments))

    if model_args is not None and data_args is not None and training_args is not None:
    ...
    train_dataset = TrainDataset(data_args, train_dataset.process(), tokenizer,
                                 dont_shuffle_negatives=training_args.dont_shuffle_negatives)
    if training_args.local_rank == 0:
    ...
    trainer = trainer_cls(
        queries_batches_path=training_args.queries_batches_path,
        model=model,
        args=training_args,
